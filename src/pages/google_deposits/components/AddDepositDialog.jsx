...
import { fetchMyAdAccounts } from "@/pages/google_ad_accounts/helpers/fetchMyAdAccounts";
import { addMoneyToGoogleAccount } from "../helpers/addMoneyToGoogleAccount";
import { accountAmounts } from "@/pages/google_ad_application/constants";
...
const AddDepositDialog = ({ open, onOpenChange }) => {
    ...
    // Fetch Active Google Accounts
    const { data: accountsData, isLoading: isLoadingAccounts } = useQuery({
        queryKey: ["myActiveGoogleAccountsForDeposit"],
        queryFn: () => fetchMyAdAccounts({ params: { status: "active", limit: 100 } }),
        enabled: open,
    });

    const activeAccounts = accountsData?.accounts || [];
    const commissionPercent = walletData?.paymentFeeRule?.google_commission || 0;

    const mutation = useMutation({
        mutationFn: (payload) => addMoneyToGoogleAccount({ id: formData.accountId, data: payload }),
        onSuccess: (data) => {
            if (data?.response?.success) {
                toast.success("Deposit request submitted successfully!");
                onOpenChange(false);
                setFormData({ accountId: "", amount: "", remarks: "" });
                queryClient.invalidateQueries(["myGoogleDeposits"]);
                queryClient.invalidateQueries(["myWallet"]);
                queryClient.invalidateQueries(["myGoogleAccounts"]);
            } else {
                ...
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 rounded-t-lg">
                    <DialogHeader>
                        ...
                        <p className="text-blue-100 mt-1.5 text-sm">
                            Add funds to your Google Ad Account.
                        </p>
                    </DialogHeader>
                </div>
