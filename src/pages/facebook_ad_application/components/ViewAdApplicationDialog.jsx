                            ...
                            <Card className="bg-zinc-900 border-zinc-800">
                                <CardContent className="p-4 space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <p className="text-xs text-zinc-500 uppercase font-medium flex items-center gap-1">
                                                <Globe className="w-3 h-3" /> Domains ({data.domainUrls?.length || 0})
                                            </p>
                                            <div className="flex flex-col gap-1">
                                                {data.domainUrls?.length > 0 ? (
                                                    data.domainUrls.map((url, i) => (
                                                        <a
                                                            key={i}
                                                            href={url}
                                                            target="_blank"
                                                    ...
                                                    <p className="text-zinc-400 italic">No domains provided</p>
                                                )}
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-xs text-zinc-500 uppercase font-medium flex items-center gap-1">
                                                <ShieldCheck className="w-3 h-3" /> License Info
                                            </p>
                                            <div className="flex flex-col">
                                                <p className="text-zinc-200 font-medium capitalize">{data.licenseType || 'N/A'}</p>
                                                <p className="text-zinc-400 text-xs">{data.licenseNumber || 'N/A'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-4 border-t border-zinc-800">
                                        <div className="space-y-1">
                                            <p className="text-xs text-zinc-500 uppercase font-medium flex items-center gap-1">
                                                <Globe className="w-3 h-3" /> Pages ({data.numberOfPages || 0})
                                            </p>
                                            <div className="flex flex-col gap-1">
                                                {data.pageUrls?.length > 0 ? (
                                                    data.pageUrls.map((url, i) => (
                                                        <a
                                                            key={i}
                                                            href={url}
                                                            target="_blank"
                                                    ...
                                                    <p className="text-zinc-400 italic">No pages provided</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-1 pt-4 border-t border-zinc-800">
                                        ...
                                        <div className="mt-1">
                                            {data.hasFullAdminAccess ? (
                                                <Badge variant="outline" className="text-green-400 border-green-400/50 bg-green-500/10">Full Admin Access</Badge>
                                            ) : (
                                                <Badge variant="outline" className="text-yellow-400 border-yellow-400/50 bg-yellow-500/10">Restricted Access</Badge>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {data.adminNote && (
                                        <div className="space-y-1 pt-4 border-t border-zinc-800">
                                            <p className="text-xs text-zinc-500 uppercase font-medium flex items-center gap-1 text-red-400">
                                                <ShieldCheck className="w-3 h-3" /> Admin Note / Rejection Reason
                                            </p>
                                            <p className="text-zinc-300 italic bg-red-500/10 p-3 rounded border border-red-500/20">"{data.adminNote}"</p>
                                        </div>
                                    )}
