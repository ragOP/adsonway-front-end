...
const ApplyAdApplicationDialog = ({ open, onOpenChange, onSuccess }) => {
  ...
  const { data: settingsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiService({
        endpoint: endpoints.getFacebookSettings,
        method: "GET",
      });
  ...
  const { data: activeAccounts, isLoading: isLoadingAccounts } = useQuery({
    ...
    queryFn: () =>
      fetchMyAdAccounts({ params: { status: "active", sort: -1 } }),
  ...
  const depositAmount = formData.adAccounts.reduce(
  ...
  const handleCountChange = (field, countField, value) => {
    ...
    if (count > currentItems.length) {
      newItems = [
        ...currentItems,
        ...Array(count - currentItems.length).fill(""),
      ];
    } else {
  ...
  const validate = () => {
    ...
    if (!formData.hasFullAdminAccess)
      newErrors.hasFullAdminAccess = "You must confirm admin access";
    ...
    formData.adAccounts.forEach((acc, index) => {
      if (!acc.accountName)
        newErrors[`adAccounts.${index}.accountName`] = "Required";
  ...
  const onSubmit = (e) => {
    ...
    const payload = {
      ...
      licenseNumber: formData.licenseNumber
        ...
        : `LIC-${Date.now().toString()}`,

      numberOfPages: formData.numberOfPages !== "0"
        ? parseInt(formData.numberOfPages)
        : 1,

      pageUrls: formData.pageUrls.length === 0 ? [`PG-${Date.now().toString()}`] : formData.pageUrls,

      hasFullAdminAccess: formData.hasFullAdminAccess ?? true,

      numberOfDomains: formData.numberOfDomains === "unlimited"
          ? "unlimited"
          : formData.numberOfDomains === "0" ? 1 : parseInt(formData.numberOfDomains),

      domainUrls:
        formData.numberOfDomains === "unlimited"
          ? []
          : formData.domainUrls.length === 0
            ? [`DM-${Date.now().toString()}`]
            : formData.domainUrls,

      numberOfAccounts: formData.adAccounts?.length || 0,

      submissionFee: totalCost,

      adAccounts:
        formData.adAccounts?.map((acc) => ({
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 rounded-t-lg">
          <DialogHeader>
            ...
            <p className="text-blue-100 mt-1.5 text-sm">
              Configure your accounts and review the billing summary before
              submission.
            </p>
          </DialogHeader>
        </div>
              ...
              <>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Existing License">
                          Existing License
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    ...
                    {formData.licenseType === "New License" ? (
                      ...
                      >
                        <SelectTrigger
                          className={`bg-white dark:bg-zinc-900 h-11 w-full ${errors.licenseNumber ? "border-red-500" : ""}`}
                        >
                          <SelectValue
                            placeholder={
                              isLoadingAccounts
                ...
                <div className="space-y-3">
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      handleCountChange("pageUrls", "numberOfPages", val)
                    }
                  >
                    <SelectTrigger className="bg-white dark:bg-zinc-900 h-11 w-full">
                    ...
                    <SelectContent>
                      {[0, 1, 2, 3, 4, 5].map((num) => (
                        <SelectItem key={num} value={num.toString()}>
                          {num === 0 ? "Select number of pages" : num}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  ...
                  {formData.pageUrls.map((url, index) => (
                    <div
                      key={index}
                      className="space-y-1 animate-in slide-in-from-top-2"
                    >
                      <Input
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 bg-zinc-50 dark:bg-zinc-900/50 border border-green-200 dark:border-green-900 rounded-md px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400 truncate flex items-center">
                      {settingsData?.facebook_profile_link ||
                        "Not set by admin"}
                    </div>
                    <Button
                      type="button"
                ...
                <div className="space-y-3">
                  <Label className="font-semibold text-sm">
                    Business Manager ID (Copy this ID) *
                  </Label>
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        copyToClipboard(
                          settingsData?.bussiness_manager_id,
                          "Business Manager ID",
                        )
                      }
                    >
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800/50 rounded-lg p-4 flex items-start gap-3">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      setFormData((prev) => ({
                        ...prev,
                        hasFullAdminAccess: checked,
                      }));
                      if (errors.hasFullAdminAccess)
                        setErrors((prev) => ({
                          ...prev,
                          hasFullAdminAccess: null,
                        }));
                    }}
                  ...
                  >
                    I've shared the Full Admin Access of the Pages to the
                    Profile Link mentioned above. *
                  </Label>
                </div>
                {errors.hasFullAdminAccess && (
                  <p className="text-xs text-red-500">
                    {errors.hasFullAdminAccess}
                  </p>
                )}
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <SelectContent>
                      {[0, 1, 2, 3, 4, 5].map((num) => (
                        <SelectItem key={num} value={num.toString()}>
                          {num === 0 ? "Select number of domains" : num}
                        </SelectItem>
                      ))}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {formData.adAccounts.map((account, index) => (
                  <Card
                    key={index}
                    className="border border-blue-100 dark:border-blue-900/30 bg-blue-50/30 dark:bg-blue-900/10 overflow-hidden transition-all hover:shadow-md"
                  >
                      ...
                      <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(val) =>
                              handleAdAccountChange(index, "timeZone", val)
                            }
                          >
                            <SelectTrigger
                              className={`h-10 w-full bg-white dark:bg-zinc-900 border-gray-200 truncate ${errors[`adAccounts.${index}.timeZone`] ? "border-red-500 ring-red-500" : ""}`}
                            >
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(val) =>
                              handleAdAccountChange(index, "amount", val)
                            }
                          >
                            <SelectTrigger
                              className={`h-10 w-full bg-white dark:bg-zinc-900 border-gray-200 ${errors[`adAccounts.${index}.amount`] ? "border-red-500 ring-red-500" : ""}`}
                            >
